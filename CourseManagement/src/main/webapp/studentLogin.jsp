...
<form action="StudentLogin" method="post" onsubmit="return validateLogin()">
<%if(!(session.getAttribute("newstudentId")==null)){
 %>
 <div style="color: green;font-weight: bold;">User Id: <%=(String)session.getAttribute("newstudentId") %></div>
 
 <% 
 session.removeAttribute("newstudentId");
} else if(!(session.getAttribute("failmsg")==null)){
	 %>
 ...
 User Id<br><input class="in"type ="text" name="userName" required><br>
...
<a href="registerstudent.jsp">Create an account</a><br>
</form>
</div>
</body>
<script>
